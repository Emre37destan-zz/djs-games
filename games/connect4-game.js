...
class ConnectFour {
    ...
    startGame (msg) {
        ...
        if(!oppenent) return msg.channel.send(`**Kiminle Connect Four oynamak istersin?**`)
        ...
        .setTitle(`🔴 ${msg.author.username} senin sıran!`)
                ...
                if(user.id === gameData[player].member.id) {
                    ...
                    switch (reaction.emoji.name) {
                        case "1️⃣":
                            ...
                            if(openSpaces.length == 0) return msg.channel.send(`**${gameData[player].member}, bu sütun zaten dolu. başka bir tane seç**`).then(msg1 => msg1.delete({timeout: 10000}))
                    ...
                    if(tieCheck()) {
                        ...
                        .setTitle(`Oyun bitti, bu Kravat!`)
                        .setDescription(renderBoard(board))
                        .setFooter(`${challenger.username} vs ${oppenent.username}`)
                        gameCollector.stop("Kravat Oyunu")
                    ...
                    for (const func of checks) {
                        ...
                        if(data) {
                            ...
                            .setTitle(`${gameData[player].member.username} oyunu kazandı!`)
                            .setDescription(renderBoard(board))
                            .setFooter(`${challenger.username} vs ${oppenent.username}`)
                            gameCollector.stop(`${gameData[player].member.id} kazandı`);
                    ...
                    .setTitle(`${gameData[player].playerColor} - ${gameData[player].member.username} senin sıran!`)
                    .setDescription(renderBoard(board))
                    .setFooter(`${challenger.username} vs ${oppenent.username}`)
